(function() {
        ...
        (function() {
            ...
            this.Emitter = function() {
                ...
                Emitter.prototype.write_double_quoted = function(text, split) {
                    ...
                    while (end <= text.length) {
                        char = text[end];
                        if (char == null || __indexOf.call('"\\\u2028\u2029﻿', char) >= 0 || !(" " <= char && char <= "~" || this.allow_unicode && (" " <= char && char <= "퟿" || "" <= char && char <= "�"))) {
                            if (start < end) {
                                data = text.slice(start, end);
                                this.column += data.length;
                                this.stream.write(data, this.encoding);
                            ...
                            if (char != null) {
                                data = char in ESCAPE_REPLACEMENTS ? "\\" + ESCAPE_REPLACEMENTS[char] : char <= "ÿ" ? "\\x" + util.to_hex(char, 2) : char <= "￿" ? "\\u" + util.to_hex(char, 4) : "\\U" + util.to_hex(char, 16);
                                this.column += data.length;
                                this.stream.write(data, this.encoding);
                                start = end + 1;
                            }
    ...
    register({
        ...
        (function() {
            var emitter, representer, resolver, serializer, util, __slice = [].slice;
            util = require("./util");
    ...
    register({
        ...
        (function() {
            var composer, constructor, parser, reader, resolver, scanner, util, __slice = [].slice;
            util = require("./util");
    ...
    register({
        ...
        (function() {
            ...
            this.serialize = function(node, stream, Dumper) {
                if (Dumper == null) {
                    Dumper = dumper.Dumper.options = {};
                }
