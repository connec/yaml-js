(function() {
  ...
  this.Emitter = (function() {
    ...
    Emitter.prototype.prepare_tag_handle = function(handle) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        char = ref[i];
        if (!(('0' <= char && char <= '9') || 'A' <= char('Z' || ('a' <= char && char <= 'z') || indexOf.call('-_', char) >= 0))) {
    ...
    Emitter.prototype.determine_block_hints = function(text) {
      var hints, ref, ref1, ref2;
      hints = '';
      if (ref = text[0], indexOf.call(' \n\x85\u2028\u2029', ref) >= 0) {
        hints += this.best_indent;
      }
      if (ref1 = text.slice(-1), indexOf.call('\n\x85\u2028\u2029', ref1) < 0) {
        hints += '-';
      } else if (text.length === 1 || (ref2 = text.slice(-2, -1), indexOf.call('\n\x85\u2028\u2029', ref2) >= 0)) {
        hints += '+';
      }
      return hints;
    };
